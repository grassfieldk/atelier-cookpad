...
export default function Home() {
  ...
  /**
   * Calculate total materials for a recipe including sub-recipes
   */
  const calculateTotalMaterials = React.useCallback(
    (recipe: CombinedRecipe): Material[] => {
      const totalMap = new Map<string, number>();

      recipe.materials.forEach((material) => {
        totalMap.set(
          material.name,
          (totalMap.get(material.name) || 0) + material.quantity,
        );
      });
  ...
  /**
   * Find recipe by name recursively
   */
  const findRecipeByName = React.useCallback(
    (
      ...
      recipe.subRecipes = recipe.materials
        .map((material) =>
          findRecipeByName(material.name, recipeList, usedRecipes),
        )
  ...
  const filteredRecipes = React.useMemo(() => {
    return recipes
      .filter((recipe) => {
        const nameMatch = normalizeKana(recipe.name).includes(
          normalizeKana(filterText),
        );
        const categoryMatch =
  ...
  const RecipeItem: React.FC<{ recipe: CombinedRecipe; level: number }> = ({
    recipe,
    level,
  }) => {
    ...
    <div>
      ...
      <div className="container max-w-screen-sm mt-[84px]">
        {filteredRecipes.map((recipe) => (
          <RecipeItem key={recipe.name} recipe={recipe} level={0} />
        ))}
      </div>
    </div>
  );
